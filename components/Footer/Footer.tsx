...
export const Footer = () => {
  ...
  const isActiveLink = (path: string): boolean => router.pathname === path
        ...
        <Box sx={styles.FooterContentBox}>
          <Box sx={styles.LeftSideBox}>
            <Typography>
              ...
              commodi illo impedit in molestias nihil nulla praesentium
              repudiandae sed, voluptatem!
            </Typography>
          ...
          <Box sx={styles.RightSideBox}>
            {headerRoutes.map((route) => (
              <Link
                ...
                sx={styles.Link(isActiveLink(route.path))}
              >
                {route.label}
              </Link>
            ))}
